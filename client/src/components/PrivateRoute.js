...
const PrivateRoute = (props) => {
    ...
    if (!user || !token) {
      ...
      verifyToken({ user, token })
        .then((/*response*/) => {
          ...
          if (props.role)
            if (user.role === props.role) setAuthenticated(1);
            else setAuthenticated(-1);
          else setAuthenticated(1);
        })
        .catch((error) => {
          console.log(error);
  ...
  //,message:'دسترسی لازم برای این صفحه را ندارید'
  else if (authenticated > 0) return <Route {...props} />;
  return <div />;
};
