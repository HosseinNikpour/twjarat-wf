...
const TableContainer = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      cc.length === 0 &&
      (props.editClick || props.displayClick || props.deleteClick)
    ) {
      cols.push({
        title: "",
        dataIndex: "operation",
          ...
          <div>
            {props.editClick && (
              <i
                ...
                onClick={() => props.editClick(record)}
              />
            )}
            {props.deleteClick && (
              <Popconfirm
            ...
            {props.displayClick && (
              <i
                ...
                onClick={() => props.displayClick(record)}
                style={{ marginRight: "8px" }}
              />
            )}
          </div>
  ...
  const onSearch = (searchText) => {
    ...
    if (searchText === "") setRows(orgRows);
    else {
      let matched = [];
      orgRows.forEach((record) => {
        columns
          .filter((a) => a.dataIndex !== "operation")
          .forEach((c) => {
  ...
  return (
    <div>
      {props.downloadName && (
        <CSVLink
          ...
          headers={props.columns
            .filter((a) => a.dataIndex !== "operation")
            .map((a) => ({ label: a.Header, key: a.accessor }))}
          filename={props.downloadName + ".csv"}
      ...
      <Table
        ...
        columns={columns}
        rowKey="id"
        scroll={props.scroll ? props.scroll : { y: 350, x: 800 }}
        bordered
        pagination={false}
      />
    </div>
