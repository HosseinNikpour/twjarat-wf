    ...
    const getData = () => {
     ...
     //   GridRef.current.scrollIntoView({ behavior: 'smooth' });
        Promise.all([ getItems(entityName)]).then((response) => {
            ...
            setParent_options(response[0].data.map(a => { return { ...a, key: a.id, label: a.title, value: a.id } }));
        })
        setObj({});;
    }
    useEffect(() => {
        ...
        <div className="card ">
            ...
            <div className='card-body' style={{ marginTop: '-50px' }}>
                ...
                <div className="form-group">
                    ...
                    <Select id='parent' className={errors.parent_id?"form-control error-control":"form-control"} {...Static.selectDefaultProp} disabled={mode === 'display'} options={parent_options}
                value={obj.parent_id} onSelect={(values) =>  setObj({ ...obj, parent_id: values})}
            />
                </div>
            </div></div>
