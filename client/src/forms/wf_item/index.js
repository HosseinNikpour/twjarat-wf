...
const WfItem = (props) => {
    ...
    const getData = () => {
        ...
        Promise.all([getItems(entityName), getItems('unit', { where: `  WHERE t.parent_id =${unit_id}` })]).then((response) => {
            setData(response[0].data);
            setReciver_options(response[1].data.map(a => { return { ...a, key: a.id, label: a.title, value: a.id } }));
        })
        setObj({});;
    }
    useEffect(() => {
            ...
            if (mode === 'new') {
                insertItem(formData, entityName, 'multipart/form-data').then(response => {
                    if (response.data.type !== "Error") {
                        ...
                        let tasks = {
                            parent_id: response.data[0].id, title: `بررسی ${obj.title}`
                            , sender_id: 1, reciver_id:obj.reciver_id
                        };
                        insertItem(tasks, "task").then(response1 => {
                            ...
                            <div className="row">
                                ...
                                <div className="col">
                                    <div className="form-group">
                                        ...
                                        <label className="req-label">*</label>
                                        <Select id='reciver_id' className={errors.reciver_id ? "form-control error-control" : "form-control"} {...Static.selectDefaultProp} disabled={mode === 'display'} options={reciver_options}
                                            value={obj.reciver_id} onSelect={(values) => setObj({ ...obj, reciver_id: values })}
                                        />
                                    </div>
                                </div>
                            </div>
